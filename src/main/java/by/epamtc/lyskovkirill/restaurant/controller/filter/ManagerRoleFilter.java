 ...
 * It is required to prevent access to URLs that can only be accessed by users with manager or admin role.
 *
 * @author k1ly
 */
public class ManagerRoleFilter implements Filter {
    ...
    public void doFilter(ServletRequest servletRequest, ServletResponse servletResponse, FilterChain filterChain) throws IOException, ServletException {
        ...
        if (userAttr == null || !(UserRole.MANAGER.equals(((UserDTO) session.getAttribute(ControllerConstants.USER)).getRoleName())
                || UserRole.ADMIN.equals(((UserDTO) session.getAttribute(ControllerConstants.USER)).getRoleName()))) {
