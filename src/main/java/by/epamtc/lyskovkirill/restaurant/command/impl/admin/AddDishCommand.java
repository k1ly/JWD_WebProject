 ...
 * for adding the {@link Dish} by admin.
 *
 * @author k1ly
 */
public class AddDishCommand implements Command {
    ...
    public CommandResult execute(HttpServletRequest request, HttpServletResponse response) throws CommandException {
        ...
        int weight = Integer.parseInt(request.getParameter(CommandConstants.WEIGHT));
        double price = Double.parseDouble(request.getParameter(CommandConstants.PRICE));
        int discount = Integer.parseInt(request.getParameter(CommandConstants.DISCOUNT));
        ...
        try {
            ...
            String imageUrl = url + "/" + imageHandler.saveImage(fileData, path);
            ...
            dish.setCategory(new DishCategory(categoryId));
            int addStatus = dishService.addDish(dish);
            if (addStatus == 0)
                session.setAttribute(ControllerConstants.ERROR_MESSAGE, "error.message.add_dish");
            else if (addStatus < 0)
                session.setAttribute(ControllerConstants.ERROR_MESSAGE, "error.message.invalid");
        } catch (ServiceException e) {
            throw new CommandException("Error during adding the dish", e);
