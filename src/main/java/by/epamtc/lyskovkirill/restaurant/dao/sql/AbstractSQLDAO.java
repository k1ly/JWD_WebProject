package by.epamtc.lyskovkirill.restaurant.dao.sql;

import by.epamtc.lyskovkirill.restaurant.dao.sql.builder.Builder;
import by.epamtc.lyskovkirill.restaurant.dao.sql.builder.BuilderProvider;
...
/**
 * Abstract class that is required to create SQL implementation of DAO interfaces.
 * Contains many methods for working directly with database.
 *
 * @author k1ly
 */
public abstract class AbstractSQLDAO<T> {
    ...
    public static final String ROW_NUMBER = "SELECT * FROM (SELECT ROW_NUMBER() OVER () row_num, ";
    public static final String PAGINATION = ") t WHERE row_num BETWEEN %d AND %d";
        ...
        if (parameters.size() > 0) {
            ...
            try {
                String sql = makeUpdateStatement(parameters.getParameters())
                        + makeQueryCondition(updateId.getParameters(), null, null, true);
    ...
    protected int softDeleteEntity(ParameterMap softDeleteId) throws SQLException {
        Map<String, Object> softDeleteParameter = new LinkedHashMap<>();
        softDeleteParameter.put(IS_DELETED, 1);
        return updateEntity(ParameterMap.of(softDeleteParameter), softDeleteId);
    }

    protected int restoreEntity(ParameterMap restoreId) throws SQLException {
        Map<String, Object> restoreParameter = new LinkedHashMap<>();
        restoreParameter.put(IS_DELETED, 0);
        return updateEntity(ParameterMap.of(restoreParameter), restoreId);
    }
    ...
    protected String makeQueryCondition(boolean includeDeleted) {
        return makeQueryCondition(null, null, null, includeDeleted);
    }

    protected String makeQueryCondition(Map<String, Object> parameters, boolean includeDeleted) {
        return makeQueryCondition(parameters, null, null, includeDeleted);
    }

    protected String makeQueryCondition(Map<String, Object> parameters, List<String> search, String searchPattern, boolean includeDeleted) {
        ...
        if (search != null) {
            for (String attr : search) {
                sqlQuery.append(attr).append(LIKE)
                        .append("'").append(searchPattern).append("'").append("%'").append(OR);
            }
            if (sqlQuery.toString().endsWith(OR))
                sqlQuery.replace(sqlQuery.lastIndexOf(OR), sqlQuery.length(), AND);
        }
        if (!includeDeleted)
            sqlQuery.append(IS_DELETED).append("=0");
    ...
    protected String makePaginationQuery(String sql, int firstRow, int rowCount) {
        return (firstRow > 0 && rowCount > 0) ? String.format(sql.replaceFirst(SELECT, ROW_NUMBER) + PAGINATION,
    ...
    protected int countTableRows(String condition) throws SQLException {
        int rowCount = 0;
        ConnectionPool connectionPool = ConnectionPool.getInstance();
        Connection connection = connectionPool.getConnection();
        try (Statement statement = connection.createStatement()) {
            ResultSet resultSet = statement.executeQuery(ROW_COUNT + tableName + condition);
            while (resultSet.next()) {
